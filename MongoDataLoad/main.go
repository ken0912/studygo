...
func init() {
	database.MustConnect("mongodb://Salarysqldev07.salarynet.local:27019", "test")
}
...
func main() {
	...
	sqlquery := `SELECT TOP 1 * FROM tbl_IncumProfile where SurveyCode = 'IPAS' and companycode = 'D28' order by LoadDate`
	...
	affectCount := database.Instance.InsertMany("tbl_IncumProfile", result)
