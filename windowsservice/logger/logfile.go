...
func (fl *FileLogger) initFile() error {
	...
	var fullname string
	var errfullname string
	if !strings.HasSuffix(fl.FilePath, "\\") {
		fullname = fl.FilePath + "\\" + fl.FileName
		errfullname = fl.FilePath + "\\" + "Err" + fl.FileName
	} else {
		fullname = fl.FilePath + fl.FileName
		errfullname = fl.FilePath + "Err" + fl.FileName
	}
	...
	errFileObj, err := os.OpenFile(errfullname, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		fmt.Println("initFile error:", err)
		return err
	}
